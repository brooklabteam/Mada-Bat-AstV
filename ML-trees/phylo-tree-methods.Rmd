...
Alignment in Geneious, modeltest using model-test-ng

Model = TVM+I+G4

MAKE THE BEAST TREE
1. generate .xml file for input into BEAST, using program BEAUti
-get dates from tip labels
-using blog, generate model TVM
...
BEAST IS DONE
1. I accidentally ran it for over 800000000 iterations, but I feel like that's fine.

Following Cara's steps, need to 1) burn in initial 10%
2) assess parameter convergence using tracer? is this only to choose between models?
3) average BEAST output in TreeAnnotator
4) visualize tree in FigTree
5) export from FigTree as Newick
6) make it pretty in R.

I think that Tracer isn't relevant now, so I'm going to skip straight to the TreeAnnotator
first step of that is pulling the output onto the desktop.

/software/beast-2.6.2-el7-x86_64/bin/treeannotator -burnin 80000000 astro_full_BEAST_alignment.trees out.txt
