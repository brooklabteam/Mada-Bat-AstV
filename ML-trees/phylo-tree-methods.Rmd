 ...
 *trimming tree to only contain the clade with our new sequence in it (based on ML phylogeny)
 17 seqs.
 *also you don't need outgroups for Bayesian trees
 
2. Alignment and model test
Alignment in Geneious using MAFFT, modeltest using model-test-ng

Model using AIC = GTR+I+G4

Partition 1/1:
                         Model         Score        Weight
----------------------------------------------------------
       BIC            TVM+I+G4   215099.4042        0.7236
       AIC            GTR+I+G4   214816.7634        0.9247
      AICc            GTR+I+G4   214816.7634        0.9247
...
-use coalescent constant population model (Lebarchenon 2018 mada bat astro paper, but RdRp?)
-clock rate lognormal with mean of 0.001 following cara following Jenkins et al 2014. all other priors left at default.
-MCMC chain length 700,000,000 and set tracelog and treelog every 10,000 iterations. all other MCMC parameters left at default.
...
--had to resample trees using LogCombiner to reduce the number of trees
--reasmpled every 100000, 10% burnin, ended up with ~14000 trees
--defaults
*finished super fast

3) make it pretty in R*
*failed! Something got messed up in the resampling/MCC/nexus export stage and I lost the posterior values, which I need to plot. Going to redo the steps above.

REDO POST PROCESSING

1) try treeannotator again with giant file
Looks like I generated 141155 trees :D oops.
I only need about 10,000.
** that means I did a total of 1411550000 iterations.

2) re-sample with LogCombiner
target ~10,000 trees. Since I did a treelog every 10,000, I need to use iterations of that. I'll try re-sampling every 100,000.
** Now I have 14,116
** if I do 25% burnin, that gets me close to 10,000 trees. 

3) Import the re-sampled into TreeAnnotator to make the MCC file
** actually you choose the number of trees for burnin, not the %, so I'm just going to say 25% of 14,116 = 3529
** keep target heights

4) open in fig tree (why Do I need this step?) I think that I want to keep it in the nexus output that TreeAnnotator generated. the file is currently in the format that the R script calls for. I'm going to skip this step.

5) make it pretty in R!
Try again reading in beast file and creating phylogeny.
