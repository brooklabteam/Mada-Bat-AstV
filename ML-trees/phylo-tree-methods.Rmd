...
GATHER SEQUENCES - 126
1. Mozambique - (31)
2. Reunion Island - (27)
3. Madagascar - not us (64)
4. Madagacsar - us (3)
5. Outgroup - (1)

ALIGNMENT
1. I aligned the RdRp sequences using MAFFT online (not on geneious anymore), exported as a .fasta

2. I uploaded the alignment back into Geneious prime to trim - final seq length 371
...
output: 
                               Model         Score        Weight
----------------------------------------------------------
       BIC            TVM+I+G4    43446.7014        0.8874
       AIC            TVM+I+G4    42444.1537        0.9975
      AICc         TPM2uf+I+G4    43572.1156        0.5909
...
`raxml-ng-mpi --check --msa SWIO_RdRp_aligned_trimmed.phy --model TVM+I+G4 --prefix T1`

9. check for number of recommended threads
`raxml-ng-mpi --parse --msa SWIO_RdRp_aligned_trimmed.phy --model TVM+I+G4 --prefix T2`
threads = 1


10. Run sbatch script starting with 800 trees

See if it converged.
`raxml-ng-mpi --bsconverge --bs-trees T3.raxml.bootstraps --prefix T3 --seed 2 --threads 1`

Did not converge - add more bootstraps with diff random seed
`raxml-ng --bootstrap -- msa SWIO_RdRp_aligned_trimmed.phy --model TVM+I+G4 --prefix T4 --seed 666 --threads 1 --bs-trees 400`

Concatenate bootstaps and check convergence now
`cat T3.raxml.bootstraps T4.raxml.bootstraps > allbootstraps`
`raxml-ng-mpi --bsconverge --bs-trees allbootstraps --prefix T5 --seed 4 --threads 1`

Converged after 950 bootstraps. 

Attached bootstraps to best tree
`raxml-ng-mpi --support --tree T3.raxml.bestTree --bs-trees allbootstraps --prefix T6 --threads 2`

Moving everything off of midway for figure making in R using T6.raxml.supports
...
*goal : make a bayesian full genome tree to infer timing of MRCA of our new genome
